...
Most tests run against SQLite and need no setup. The `ModePgx` tests need a real
PostgreSQL server and skip unless `GAS_TEST_POSTGRES_DSN` is set:

```bash
docker run -d --name gas/db-test -e POSTGRES_PASSWORD=test -e POSTGRES_DB=gastest -p 5433:5432 postgres:16-alpine

GAS_TEST_POSTGRES_DSN="postgres://postgres:test@localhost:5433/gastest?sslmode=disable" go test -race ./...
```

The tests create and drop their own scratch tables, so any throwaway database
works. Please run them before changing anything under `ModePgx`.
